...
def main():
    ...
    scalarized_sol = solve_scalarized(inst, lambda_c=50.0, lambda_w=50.0, output_flag=0, existing_sites=EXISTING_SITES)
    ...
    # 19. Site flows stacked (inference vs training) - improved 3-panel layout
    if baseline_sol and capped_sol:
        print("\n19. Generating stacked flows plot (inference vs training)...")
        from src.plots_new import plot_site_flows_stacked_inf_train
        ...
        def solution_to_dict(sol):
            ...
            return {
                ...
                'gamma_co2': getattr(sol, 'gamma_co2', None),
                'gamma_w': getattr(sol, 'gamma_w', None),
            }
        
        baseline_dict = solution_to_dict(baseline_sol)
        capped_dict = solution_to_dict(capped_sol)
        scalarized_dict = solution_to_dict(scalarized_sol)
        
        plot_site_flows_stacked_inf_train(inst, baseline_dict, capped_dict, scalarized_dict, str(figures_dir))
        print(f"   Saved to {figures_dir / 'fig_site_flows_stacked_inf_train.png'}")
